...
var ClipboardItemStub = class {
  ...
  getType(type3) {
    return __async(this, null, function* () {
      const data = yield this.data[type3];
      if (!data) {
  ...
  readText() {
    return __async(this, null, function* () {
      ...
      for (const item of this.items) {
        ...
        if (type3) {
          text += yield item.getType(type3).then((b) => readBlobText(b));
        }
      }
      return text;
    });
  ...
  return __async(this, null, function* () {
    ...
    for (const item of items) {
      for (const type3 of item.types) {
        dt.setData(type3, yield item.getType(type3).then((b) => readBlobText(b)));
      }
    }
    return dt;
  });
...
function isVisible(element) {
  const window2 = getWindowFromNode(element);
...
import { getWindowFromNode as getWindowFromNode2 } from "@testing-library/dom/dist/helpers.js";
...
function hasPointerEvents(element) {
  const window2 = getWindowFromNode2(element);
  for (let el = element; el == null ? void 0 : el.ownerDocument; el = el.parentElement) {
...
function paste(clipboardData) {
  return __async(this, null, function* () {
    ...
    const doc = this[Config].document;
    const target = (_a = doc.activeElement) != null ? _a : doc.body;
    const dataTransfer = (_b = typeof clipboardData === "string" ? getClipboardDataFromString(clipboardData) : clipboardData) != null ? _b : yield readDataTransferFromClipboard(doc).catch(() => {
